...
## 2026-08-12
### v2.8.0
- 接入 IGXE 数据源：商品查询、下单、在售列表、订单详情获取，消息盒子同步与自动采集调度
- 在售页、库存页接入 IGXE Tab，交易数据页面支持识别 IGXE 来源
- 完成 BUFF 平台全链路接入：库存上架出售/出租、在售列表改价与下架、预售与过户下架改价、求购、购买、租出、消息页同步，手续费率收敛到统一常量
- 在售页阻断改价/下架的错误路由，BUFF 模块补全子类型并修复列表加载 405
- 新增数据库(MySQL)迁移维护模式，迁移期间加锁防止并发数据库操作，优化连接测试反馈与迁移错误提示
- 优化数据库查询排序逻辑，修复主键冲突检查，增强错误处理
- 新增 CSQAQ 历史交易数据获取（开发者工具）
- 优化图片下载方式
- 修复出租页筛选查错表导致非悠悠平台无数据，出租页普通加载改走筛选接口
