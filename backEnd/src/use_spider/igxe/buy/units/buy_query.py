...
class BuyQuery:
    ...
    def count_data(user_id):
        """统计指定用户 IGXE 平台购买记录数量（按主订单号去重，与列表分页粒度一致）"""
        try:
            db = DatabaseManager()
            rows = db.execute_query(
                'SELECT COUNT(DISTINCT ID_sub) FROM buy WHERE data_user = ? AND "from" = ?',
                (user_id, "igxe"),
            )
            count = rows[0][0] if rows else 0
            return jsonify({"count": int(count or 0)}), 200
        except Exception as exc:
            print(f"统计 IGXE 购买数据失败: {exc}")
            return jsonify({"count": 0}), 500

    @staticmethod
    def get_latest_data(user_id):
        """获取指定用户 IGXE 平台最新一条购买记录（主订单号和订单时间）"""
        try:
    ...
    def select_not_end(user_id):
        """查询 IGXE 平台未完成的购买主订单号列表"""
        try:
            db = DatabaseManager()
            rows = db.execute_query(
                'SELECT DISTINCT ID_sub FROM buy '
